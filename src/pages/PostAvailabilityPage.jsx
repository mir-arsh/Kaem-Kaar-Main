...
const PostAvailabilityPage = () => {
  ...
  return (
    <AppShell header={<h2 className="font-bold text-foreground">Post Availability</h2>}>
      <div className="px-4 py-6 space-y-5">
        <div className="bg-primary/10 border border-primary/20 rounded-2xl px-4 py-3">
          <p className="text-xs font-bold text-primary">
            Let hirers know you're available for work. They'll be able to see your profile and contact you.
          </p>
        </div>

        <div className="space-y-1">
          <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Your Skill</label>
          <select
            value={skill}
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Expected Pay / Day (₹)</label>
          <Input type="number" value={pay} onChange={(e) => setPay(e.target.value)} className="h-14 rounded-xl" placeholder="500" />
        </div>

        <div className="space-y-1">
          <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Available From (optional)</label>
          <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="h-14 rounded-xl" />
        </div>

        <div className="space-y-1">
          <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Note (optional)</label>
          <Textarea value={note} onChange={(e) => setNote(e.target.value)} className="rounded-xl min-h-[80px]" placeholder="e.g. 5 years experience..." />
        </div>

        <Button onClick={handlePost} disabled={loading} className="w-full h-14 rounded-xl">
          {loading ? <Loader2 className="animate-spin mr-2" size={18} /> : "Post Availability"}
